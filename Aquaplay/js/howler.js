...
(function() {
  ...
  HowlerGlobal.prototype = {
	...
	volumeGroup: function(group,volume) {
		if(!groups[group])
			groups[group] = new MusicGroup(group);
		groups[group].state = volume;
	
		for(var i in groups[group].howls) {
			var h = groups[group].howls[i];
			h.volume(volume);
		}
	},
	groupState: function(group) {
		if(groups[group]) return groups[group].state;
		return null;
	},
	...
	removeHowlFromGroup: function(group, howl) {
		...
		for(var i in groups[group].howls) {
			if(groups[group].howls[i] == howl) {
				groups[group].howls.splice(i,1);
			}
		}
	},
    /**
  ...
  var Howl = window.Howl = function(o) {
	...
	/*for(var i in groups) {
		if(i == o._group) {
			if(!groups[i])
				groups[i] = new MusicGroup(o._group);
			groups[i].howls.push(this)
		}
	}*/
	...
	if(self._group != "") {
		...
		//if(groups[self._group].state === false) {
			self.volume(groups[self._group].state);
		//}
	}
  };
